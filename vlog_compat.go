...
func (d vlogProducer) ErrorString(msgs ...interface{}) string {
	d.hcl.Error(joinInterfaces(msgs...))
	return ""
}
...
func (d vlogProducer) Trace(fnName string) (string, func() string) {
	traceFunc := func() string {
		d.hcl.Trace(fmt.Sprintf("END %s", fnName))
		return ""
	}

	d.hcl.Trace(fmt.Sprintf("START %s", fnName))
	return "", traceFunc
}
